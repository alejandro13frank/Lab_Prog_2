...
namespace _20191121_SP
{
    public partial class FrmSistemaSolar : Form
    {
        ...
        {
            string nombre = "... Franco Rodrigo Disconsi 2C.";
            if(this.animaciones.Count == 0)
            {
                foreach(Planeta p in this.planetas)
                {
                    Thread aux = new Thread(p.AnimarSistemaSolar);
                    this.animaciones.Add(aux);
                }
                // Iniciar hilos
                ...
                this.Text = nombre.EstadoSimulacion(true) + nombre;
            }
            else
            {
                this.LimpiarAnimaciones();
                this.btnSimular.Text = "Simular";
                this.Text = nombre.EstadoSimulacion(false) + nombre;
            }
        }
